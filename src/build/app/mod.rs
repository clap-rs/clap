...
impl<'b> App<'b> {
    ...
    /// Adds an [`Arg`] to the application, or if it already exists, update the existing one.
    ///
    /// This is most useful to modify an application after it has already been generated, such as
    /// through procedural macros.
    ///
    /// # Examples
    ///
    /// Suppose an application was allowed to pass arbitrary arguments to an external program. This
    /// would require enabling the `AllowHyphenValues` argument setting.
    ///
    /// However, if the [`App`] was generated by another crate or by `structopt`, the setting
    /// could not be set on an argument that already existed. This function facilitates that use
    /// case:
    ///
    /// ```rust
    /// # use clap::{App, Arg, ArgSettings};
    /// # fn generate_app() -> App<'static> {
    /// #     App::new("app")
    /// #         .arg(
    /// #             Arg::with_name("command-args")
    /// #                 .long("ext-arg")
    /// #                 .takes_value(true)
    /// #                 .multiple(true))
    /// #         .arg(
    ...
    /// # }
    /// ```
    pub fn arg_add_or_modify<FnArg>(mut self, arg_name: &'b str, arg_update_fn: FnArg) -> Self
    where
        FnArg: Fn(Arg) -> Arg,
    {
        // Since several of these panic, this requires odd handling.
        //
        // The algorithm basically adds a duplicate then tries to remove it, and figures out if
        // it was the same item or not after the fact.

        let new_index = self.args.push(Arg::with_name(arg_name));
        let actual_index = self.args.find_by_name(arg_name);

        // Do the first switch: insert a dummy to preserve order.
        let dummy_arg = Arg::with_name(arg_name);
        let actual_arg =
            std::mem::replace(self.args.args.get_mut(actual_index).unwrap(), dummy_arg);

        // Run the actual function.
        let processed_arg = (arg_update_fn)(actual_arg);

        // Switch it back, and throw away the dummy.
        std::mem::replace(self.args.args.get_mut(actual_index).unwrap(), processed_arg);

        // Finally, figure out if our duplicate is still there.
        if new_index == actual_index {
            // It's not, nothing to do.
        } else {
            // It's in there, so remove it.
            assert!(self.args.args.pop().unwrap().name == arg_name);
        }
