...
impl<'a, 'b> App<'a, 'b> {
    ...
    /// [`App::write_long_help`] or [`App::write_short_help`] instead if possible!
    ///
    /// # Examples
    ///
    /// ```rust
    ...
    /// the user ran `-h`
    /// 
    /// **NOTE:** it was introduced to fix a bug with [`App::write_help`] without breaking
    /// compatibility
    ///
    /// **NOTE:** clap has the ability to distinguish between "short" and "long" help messages
    /// depending on if the user ran [`-h` (short)] or [`--help` (long)]
    ...
    /// let mut app = App::new("myprog");
    /// let mut out = io::stdout();
    /// app.write_short_help(&mut out).expect("failed to write to stdout");
    /// ```
    /// [`io::Write`]: https://doc.rust-lang.org/std/io/trait.Write.html
    /// [`-h` (short)]: ./struct.Arg.html#method.help
    /// [`--help` (long)]: ./struct.Arg.html#method.long_help
    pub fn write_short_help<W: Write>(&mut self, w: &mut W) -> ClapResult<()> {
        self.p.propagate_globals();
        self.p.propagate_settings();
        self.p.derive_display_order();
        self.p.create_help_and_version();
