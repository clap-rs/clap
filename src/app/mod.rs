    ...
    'a: 'b,
{
    ...
    #[doc(hidden)]
    pub arg_headings: Vec<Option<&'a str>>,
}
...
impl<'a, 'b> App<'a, 'b> {
    ...
    pub fn arg<A: Into<Arg<'a, 'b>>>(mut self, a: A) -> Self {
        let arg_heading : Option<&'a str> = if let Some(option_str) = self.arg_headings.last() {
            *option_str
        } else {
            None
        };
        let arg = a.into().arg_heading(arg_heading);
        self.args.push(arg);
        self
    }
