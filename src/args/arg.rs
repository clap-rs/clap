    ...
    pub fn from_yaml<'y>(y: &'y BTreeMap<Yaml, Yaml>) -> Arg<'y, 'y> {
        ...
        for (k, v) in arg_settings.iter() {
            a = match k.as_str().unwrap() {
                ...
                "takes_value" => a.is_set(ArgSettings::TakesValue)(v.as_bool().unwrap()),
                "index" => a.index(v.as_i64().unwrap() as u8),
                "global" => a.is_set(ArgSettings::Global)(v.as_bool().unwrap()),
                "multiple" => a.is_set(ArgSettings::Multiple)(v.as_bool().unwrap()),
