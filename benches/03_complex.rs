...
fn create_app_builder(b: &mut Bencher) {
    b.iter(|| {
        App::new("claptests")
            ...
            .arg(Arg::new("positional3")
                     .multiple(true)
                     .help("tests positionals with specific values")
                     .index(4)
                     .possible_values(&POS3_VALS))
            .arg(Arg::new("multvals")
                     .long("multvals")
                     .takes_value(true)
...
fn parse_complex1(b: &mut Bencher) {
    b.iter(|| {
        create_app!().get_matches_from(vec!["myprog",
                                            "-ff",
                                            "-o",
                                            "option1",
                                            "arg1",
...
fn parse_sc_complex(b: &mut Bencher) {
    b.iter(|| {
               create_app!().get_matches_from(vec!["myprog", "subcmd", "-f", "-o", "option1",
                                                   "arg1"])
           });
}
