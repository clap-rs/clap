...
fn parse_sc_flag(b: &mut Bencher) {
    b.iter(|| {
        create_app!().get_matches_from(vec!["myprog", "subcmd", "-f"])
    });
}
...
fn parse_sc_option(b: &mut Bencher) {
    b.iter(|| {
        create_app!().get_matches_from(vec!["myprog", "subcmd", "-o", "option1"])
    });
}
...
fn parse_sc_positional(b: &mut Bencher) {
    b.iter(|| {
        create_app!().get_matches_from(vec!["myprog", "subcmd", "arg1"])
    });
}
