...
/// Build the `cmd` value the dispatch loop holds once `path` has been typed.
///
/// The two halves escape hyphens differently and that is not a typo. The bin
/// name is escaped as `-` to `__` in `try_generate`, while each subcommand is
/// escaped as `-` to `CMD_SEP` in `add_command`. Escaping the path in one pass
/// produces a label the loop never sets.
fn bash_state_for_path(path: &str) -> String {
    let mut components = path.split(' ');
    let bin_name = components.next().unwrap_or_default();

    let mut state = bin_name.replace('-', "__");
    for subcommand in components {
        state.push_str(CMD_SEP);
        state.push_str(&subcommand.replace('-', CMD_SEP));
    }
    state
}
